...
// ─────────────────────────────────────────────────────────────────────────────
// Receives a POST from the "Health Auto Export" iOS app containing HealthKit
// data as JSON, transforms it into dashboard-ready shape, and stores it in
// Vercel KV so the PWA can fetch it on demand.
//
// Set these environment variables in your Vercel project dashboard:
//   WEBHOOK_SECRET   — any random s// api/health-webhook.js
...
tring; paste the same value into the
//                      Health Auto Export app's "Authorization" header field
//   KV_REST_API_URL  — auto-set when you add Vercel KV to your project
//   KV_REST_API_TOKEN— auto-set when you add Vercel KV to your project
// ─────────────────────────────────────────────────────────────────────────────
...
export default async function handler(req, res) {
  ...
  // Verify the secret header so only your iPhone can write data
  const secret = req.headers["x-webhook-secret"] || req.headers["authorization"];
  if (secret !== process.env.WEBHOOK_SECRET) {
    return res.status(401).json({ error: "Unauthorized" });
  }
  ...
  try {
    ...
    // Health Auto Export sends an array of metric objects like:
    // { name: "Workouts", units: "...", data: [ { date, qty, ... } ] }
    // We pull out the pieces we need for the dashboard.
    ...
    const heartRateMetric  = find("heart rate");
    const restingHRMetric  = find("resting heart rate");
    const distanceMetric   = find("walking");
    ...
    const runs = allWorkouts
      ...
      .map((w) => ({
        date:     w.startDate || w.date,
        distance: Math.round((parseFloat(w.totalDistance) || 0) * 1000),  // km→m
        duration: Math.round(parseFloat(w.duration) || 0),                 // seconds
        calories: Math.round(parseFloat(w.totalEnergyBurned) || 0),
      }))
      .sort((a, b) => new Date(b.date) - new Date(a.date));
    ...
    // YTD miles (2026 runs only, from tracked workouts)
    const ytdMeters = runs
      .filter((r) => r.date?.startsWith("2026"))
      .reduce((sum, r) => sum + r.distance, 0);
    const ytdMiles = parseFloat((ytdMeters / 1609.34).toFixed(1));
    ...
    for (let i = 6; i >= 0; i--) {
      ...
      last7Days.push({
        date:   label,
        miles:  runDates.has(key)
                  ? parseFloat((runs.find((r) => r.date?.startsWith(key))?.distance / 1609.34).toFixed(2))
                  : 0,
        hr:     hrByDay[key] || null,
        hasRun: runDates.has(key),
      });
    ...
    const longestRun  = runs2026.reduce((best, r) => r.distance > (best?.distance || 0) ? r : best, null);
    const fastestRun  = runs2026.reduce((best, r) => {
      const pace = r.duration / (r.distance / 1609.34);
      const bestPace = best ? best.duration / (best.distance / 1609.34) : Infinity;
      return pace < bestPace ? r : best;
    }, null);
    const mostCalRun  = runs2026.reduce((best, r) => r.calories > (best?.calories || 0) ? r : best, null);
    ...
    const prs = [
      longestRun  ? { label: "Longest Run",     value: fmtMiles(longestRun.distance),               date: fmtDate(longestRun.date),  note: `${longestRun.distance} m` }            : null,
      fastestRun  ? { label: "Fastest Pace",    value: fmtPace(fastestRun.duration, fastestRun.distance), date: fmtDate(fastestRun.date), note: fmtMiles(fastestRun.distance) }     : null,
      mostCalRun  ? { label: "Most Calories",   value: mostCalRun.calories + " kcal",               date: fmtDate(mostCalRun.date),  note: fmtMiles(mostCalRun.distance) }          : null,
    ].filter(Boolean);
    ...
    (restingHRMetric?.data || []).forEach((h) => {
      ...
      if (month && (!restingByMonth[month] || val < restingByMonth[month])) {
        restingByMonth[month] = val;
      }
    });
    const bestRestingEntry = Object.entries(restingByMonth)
      .filter(([m]) => m.startsWith("2026"))
      .sort((a, b) => a[1] - b[1])[0];
    if (bestRestingEntry) {
      const [monthKey, bpm] = bestRestingEntry;
      const label = new Date(monthKey + "-01").toLocaleDateString("en-US", { month: "long", year: "numeric" });
      prs.push({ label: "Best Resting HR", value: Math.round(bpm) + " bpm", date: label, note: "Monthly avg resting HR" });
    }
    ...
    const restingVals = (restingHRMetric?.data || []).map((h) => parseFloat(h.qty)).filter(Boolean);
    const avgResting  = restingVals.length ? Math.round(restingVals.reduce((a, b) => a + b, 0) / restingVals.length) : null;
    const lowestResting = restingVals.length ? Math.round(Math.min(...restingVals)) : null;
    const peakHR      = hrVals.length ? Math.round(Math.max(...hrVals)) : null;
    ...
    const dashboardData = {
      ...
      monthlyData,
      prs,
      hrSummary: { avgHR, avgResting, lowestResting, peakHR },
      hrMonthly: [], // populated below
      hrDaily:   (heartRateMetric?.data || []).slice(-30).map((h) => ({
        date: new Date(h.date).toLocaleDateString("en-US", { month: "short", day: "numeric" }),
        hr:   Math.round(parseFloat(h.qty) || 0),
      })),
    };
    ...
    (heartRateMetric?.data  || []).forEach((h) => { const m = h.date?.slice(0,7); if(m){ if(!hrMonthMap[m]) hrMonthMap[m]={avg:[],rest:[]}; hrMonthMap[m].avg.push(parseFloat(h.qty)); } });
    (restingHRMetric?.data  || []).forEach((h) => { const m = h.date?.slice(0,7); if(m){ if(!hrMonthMap[m]) hrMonthMap[m]={avg:[],rest:[]}; hrMonthMap[m].rest.push(parseFloat(h.qty)); } });
    dashboardData.hrMonthly = Object.entries(hrMonthMap).sort().map(([k,v]) => ({
      month:   new Date(k+"-01").toLocaleDateString("en-US",{month:"short"}),
      avg:     v.avg.length  ? Math.round(v.avg.reduce((a,b)=>a+b,0)/v.avg.length)   : null,
      resting: v.rest.length ? Math.round(v.rest.reduce((a,b)=>a+b,0)/v.rest.length) : null,
    }));
    ...
    // ── Save to Vercel KV ─────────────────────────────────────────────────────
    await kv.set("dashboard", dashboardData);
